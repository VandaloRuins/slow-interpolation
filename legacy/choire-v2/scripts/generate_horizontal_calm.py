"""
generate_horizontal_calm.py -- Calm fork of generate_horizontal.py.
Lower denoising, less noise injection, stronger temporal smoothing.
The scene morphs slowly and organically instead of actively moving.

Usage:
    python generate_horizontal_calm.py --subject harbour
    python generate_horizontal_calm.py --subject notturno --tag v2
"""
...
def main():
    ...
    parser.add_argument("--smooth-sigma", type=float, default=1.8, help="Temporal smoothing sigma (calm default: 1.8)")
    ...
    name = f"{subject['name']}_horizontal_calm{tag}"
    ...
    # CALM: override noise/decay for slower movement
    NOISE_BLEND_CALM = 0.04           # half of normal 0.08
    NOISE_BLEND_TRANSITION_CALM = 0.08  # half of normal 0.15
    STRUCTURAL_DECAY_RADIUS_CALM = 0    # no pre-blur (was 2) - prevents focus loss
    NOISE_WALK_RATE_CALM = 0.02         # slower noise evolution (was 0.05)
    SMOOTH_SIGMA_DEFAULT = 1.8          # mild temporal smoothing (was 1.5, 2.5 too blurry)
    ...
    # CALM: moderate denoising (sharp frames) but low noise = slow evolution
    ...
    for wi in range(WARMUP_FRAMES):
        ...
        input_img = evolved_noise_blend(current_img, blend_pct=NOISE_BLEND_CALM, walk_rate=NOISE_WALK_RATE_CALM)
        result = pipe(
    ...
    for seg_idx in range(n_segments):
        ...
        for fi in range(STEADY_FRAMES):
            ...
            input_img = evolved_noise_blend(structural_decay(current_img, STRUCTURAL_DECAY_RADIUS_CALM), blend_pct=NOISE_BLEND_CALM, walk_rate=NOISE_WALK_RATE_CALM)
            result = pipe(
                prompt_embeds=embeds_cur, pooled_prompt_embeds=pooled_cur, **cfg_kwargs,
                image=input_img, strength=strength,
                num_inference_steps=NUM_INFERENCE_STEPS, guidance_scale=GUIDANCE_SCALE,
        ...
        for ti in range(actual_trans_n):
            ...
            if is_return:
                ...
                strength = 0.48 + progress * 0.07  # 0.48 -> 0.55 (calmer return)
                ...
                strength = args.transition_strength if args.transition_strength > 0 else 0.52

            trans_noise = NOISE_BLEND_TRANSITION_CALM
            input_img = evolved_noise_blend(
                structural_decay(current_img, STRUCTURAL_DECAY_RADIUS_CALM),
                blend_pct=trans_noise, walk_rate=NOISE_WALK_RATE_CALM)
