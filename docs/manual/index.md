...
| Stand up a gallery so a **student** can QA **dataset images** | [gallery.md](gallery.md) | Same gallery server works on any `datasets/<name>/` folder. The HTML rebuild is keyword-substitution on the Renoir template. |
| **Browse or share a dataset's current state, read-only** (finals only, staged crops applied, searchable audit + curation tags) | [dataset-glance.md](dataset-glance.md) | The triage and inspection sibling of the curation gallery, on the Glance viewer. One command rebuilds it from the dataset's own files; it never writes `datasets/<name>/`. It has NO verbs (no remove, crop, flag, export), so it is never the Phase 3 review surface. |
| Show finished **renders** (MP4s, keyframe PNGs) to the **maintainer** | [render-gallery.md](render-gallery.md) | Different tool, same word: `tools/gallery.py` over `outputs/`. Delivery-spec badges, web proxies for phone playback, and the `gallery-feedback.json` round-trip that carries the maintainer's notes back to you. The page is a build artifact and nothing rebuilds it for you. |
...
| **Change what the shared Glance field looks like or does** (a layout problem, a missing control, a new curation affordance) | [glance-viewer.md](glance-viewer.md) | **Read before touching anything the viewer renders.** The viewer is a released dependency this repo does not vendor, so an edit to a built field is destroyed by the next install. The page decides for you whether a change belongs upstream or in this repo's injected layer, and documents that injection seam plus its two compatibility asserts. |
...
6b. **Showing finished renders to the maintainer:** [render-gallery.md](render-gallery.md). The other gallery. What the cards show, the feedback round-trip, the caches, and why the page being a build artifact is the failure that recurs most.
6c. **Browsing a dataset read-only (finals, crops applied):** [dataset-glance.md](dataset-glance.md). The third display surface. Which of the three to reach for, how to build and serve it, and why it can never host the student's Phase 3 actions.
