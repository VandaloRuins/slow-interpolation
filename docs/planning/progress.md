...
- **Content complexity, not the LoRA, is what a chained img2img walk cannot hold** (2026-08-08, decisive negative result). Ten Course of Empire renders. Stages drawn from Cole's repertoire (storm, pasture, ruins) GAIN sharpness across the chain; a modern city and a burning city LOSE it under every setting tried. v10 halved `lora_scale` on the city stages only and instability changed by **0%** (per-frame change 1.234 -> 1.235), so SDXL base is exactly as unstable on a dense city as the Cole LoRA is. What worked instead was reducing the NUMBER of elements: v9 cut the skyline from twelve towers to five masses and scored the best image quality of the run. Full write-up in [findings/chained-diffusion-limits.md](../findings/chained-diffusion-limits.md), which also records the coupled-dial rules (steps vs strength, keyframes vs strength, keyframes vs decay), the two distinct rhythms (sharpness pulse vs velocity stepping, traded by `skip_boundary`), and five dead ends not worth re-running. *[Correction 2026-08-10: the v10 leg was a no-op. Per-segment `lora_scale` never reached the model under ControlNet (`cross_attention_kwargs` silently discarded; fixed in `079598d`), so the "not the LoRA" attribution is untested pending a rerun. The trained-vs-untrained observation and the v9 fewer-larger-masses result stand. See quality-first L16.]*

- **Frame-average metrics are not sufficient to judge a render** (2026-08-08). A single sharpness number per clip hides temporal structure (a 30% pulse once per keyframe pair), regional structure (a dense city carries the average while sky and water stay soft) and per-stage structure (sharp for 20 s, soft for 50 s reports as fine). Laplacian variance is additionally confounded by noise: `structural_decay_radius: 0` scored 3.5x on it while reading as a digital render. Always measure per stage, cross-check with an FFT high-frequency ratio, and treat the perceptual review as the arbiter of whether something looks good. New tools: `tools/pulse_plot.py`, `tools/motion_profile.py`, `tools/analyse_ladder.py`, `tools/gemini_review.py`.

- **Quality-first workstream opened, and it outranks the billboard** (2026-08-07). Luca: "I am not satisfied with current slow interpolation outputs and want better outputs, so this research phase leading to a better-tested direction is a priority." The NYC billboard's three delivery files are banked and spec-verified, so nothing ships on a deadline; the billboard is now downstream of the technique's output quality rather than the driver of it. All learnings, open directions and the task board live in one file: [workstreams/quality-first/progress.md](workstreams/quality-first/progress.md). Read it before touching the render stack.

- **Every keyframe ever rendered got 2 denoising steps** (2026-08-07). `num_inference_steps` was hardcoded at 4 in `generate_keyframes` and never passed from `pipeline.py`, so it was unreachable from YAML and was never swept. Diffusers img2img runs `int(num_inference_steps * strength)`, and steady strength is 0.55, so `int(4 * 0.55) = 2`. Confirmed in code and in a live Modal container log. Applies to all three banked billboard deliverables and every render before them. This also reframes the 2026-05-18 FLUX-rejection follow-up ("is Lightning bottlenecking our LoRAs' brushwork"): that probe rendered Lightning at its full 4 steps, so the production chain sits below the worst case it measured. **`SamplingConfig` shipped the same day** (`num_inference_steps`, `guidance_scale`), plus `ModelsConfig.lightning_lora: null` to skip the Lightning fuse and its trailing-timestep scheduler swap together, and `ModelsConfig.vae_kind` + `vae_full` to swap TAESD for `AutoencoderKL`. Backwards compatible, 60 tests pass. See [findings/denoise-step-budget.md](../findings/denoise-step-budget.md).

- **`strength` and `num_inference_steps` are separable, and only `strength` is a drift control** (2026-08-07). `strength` sets the re-entry point in the noise schedule (the amount of change); `num_inference_steps` sets how finely that same distance is traversed (the quality). Raising steps at fixed strength buys model authority at identical drift rate. This is close to a free quality dial and was invisible because neither was YAML-addressable.

- **Cole is an expressionist-preset LoRA with a text-encoder arm, not a floral-preset one** (2026-08-07, read off the safetensors headers). Thomas Cole ep10: rank 32 / alpha 32 / LR 5e-4, 986 modules including 792 text-encoder keys, 218 MB. Soutine ep10: rank 32 / alpha 32 / LR 5e-4, UNet-only, 163 MB. Renoir ep10: rank 16 / alpha 8 / LR 3e-5, 81 MB. Consequence: Cole and Soutine share an identical `alpha/rank`, so **Soutine at a given `lora_scale` is not "hotter" than Cole at the same scale**, and any reasoning that assumed Cole was the floral preset is void. Cole is also the only checkpoint with a text-encoder arm, which `_load_style_lora`'s Kohya `IndexError` fallback probably discards at load time; unverified at runtime. Also recorded: **Cole's `lora_scale` of 0.75 has never been swept and has no decisions-log entry**; it is inherited unchanged from the legacy After Cole pipeline via `tcole_valley.yaml`.

- **StreamDiffusion's headline feature does not transfer; one 30-line idea does** (2026-08-07, anatomy pass on the local checkout). Stream Batch is structurally unusable for this pipeline: its shift register holds partially denoised states of the previous N-1 *external* frames and every consumer feeds an external source, whereas our chain is a feedback loop. RCFG cannot be lifted piecemeal (`stock_noise` is written at three sites across two methods and converges only under stream continuity). The canonical package has **zero SDXL support**; SDXL exists only in the vendored `StreamDiffusionTD/` TouchDesigner fork, which is a usable reference if the live path is ever built. The one worthwhile steal is `t_index_list`: denoise expressed as an explicit list of scheduler timesteps to visit rather than a strength scalar, which decouples entry point from step count and allows non-uniform spacing. Build it only after the step ladder says whether step count matters.

- **Modal was never uninstalled; the billboard handover was wrong** (2026-08-07). It is installed under Python 3.11 with the CLI on PATH, profile `vandaloruins`, authenticated, and `modal run -m cloud.X` works with zero setup. The repo's active 3.10 interpreter is the one that cannot `import modal`. Root cause of the false claim is very likely a doc bug in our own manual: `docs/manual/modal-operations.md` lines 135 and 163 instruct agents to run `modal token current`, which **does not exist in modal 1.4.2**, so a healthy account reads as unauthenticated. Working command is `modal profile current`. Spend is $0.00 this month against $21.00 in May; fitted cost model is 36 s fixed + 2.3 s per keyframe at 1344x768 on L40S; all weights including FLUX.1-dev and FLUX.1-schnell are already on the volumes. Smoke re-verified at $0.0183 with no image rebuild.
