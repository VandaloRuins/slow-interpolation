...
| harness | `tools/agent-ops-harness/**`, `agent-ops-harness.config.json`, `docs/planning/*-registry.md` | `docs/planning/progress.md` | one-line entry under the current session paragraph | parent chat | yes | installed 2026-08-07; source-of-truth Tier 2 corrected 2026-08-14 |
| glance / SI platform | `tools/glance_*`, `tools/serve_glance.py`, `tools/inbox_push.py`, `tools/decisions_pull.py`, `tools/glance_queue.py`, `.claude/skills/glance-gallery/**`, `.glance-*-vercel.json`, `docs/planning/glance-*.md`, `docs/media-archive-*.md` | `docs/planning/glance-gallery.md` `## Open work` (the SI-N table) | numbered `SI-N` row; closed items get `~~SI-N~~` plus the verification evidence in the same cell | parallel chat (the SI Glance session) | **no** (hand off) | live: SI-1/SI-2 closed 2026-08-14, SI-14 built. Row added 2026-08-14 to stop this subtree falling through to `(default)`, the gap DT13 opened |
