...
| Environment setup and dependency versions | `docs/dev-setup.md`, `docs/dependencies.md` | Canonical, but defer to **`pyproject.toml`** for the actual pinned versions and extras. Since 2026-08-14 `requirements.txt` is a POINTER (`-e .[dev]`), not an independent list: it was a hand-maintained parallel list and the two disagreeing over opencv is what held CI red for 28 runs. Never reconcile the two against each other; there is one list. |
