"""Build a self-contained mosaic gallery of the Renoir dataset.
...
<head>
<meta charset="utf-8">
<title>Renoir Flowers Dataset (__COUNT__ images)</title>
<style>
  :root {
    --bg: #0e0b08;
    --ink: #f0e8d8;
...
<div id="file-warning" style="display:none; background:#3a2a1a; color:#ffc88a; text-align:center; padding:12px 16px; font-size:13px; letter-spacing:0.04em; gap:14px; align-items:center; justify-content:center; flex-wrap:wrap;">
  ...
  <code id="file-warning-cmd" style="display:none; color:#ffd9a8;">py -3.11 datasets/renoir-flowers/serve.py</code>
...
<header>
  <h1>Renoir Flowers <em>— Dataset Mosaic</em></h1>
  <p>__COUNT__ floral paintings sourced from Wikimedia Commons for the <code style="color:var(--accent);font-family:inherit;font-style:italic">rfl</code> SDXL LoRA</p>
...
<main id="grid"></main>
<footer>
  <code>slow-interpolation / Renoir LoRA training set</code>
</footer>
...
const LS_KEY = "renoir-gallery-flags-v1";
const LS_CROPS = "renoir-gallery-crops-v1";
...
for (const c of CARDS) {
  // The caption is the full "rfl, ..." sentence; split off the first segment
  // ("rfl, [subject]") for the trigger styling.
  const cap = c.caption || "";
  const [firstSeg, ...restSegs] = cap.split(", ");
  // firstSeg is "rfl" alone; subject begins at restSegs[0]
    ...
    im.onload = () => {
      ...
      try {
        ...
        reject(new Error(isFile
          ? `Canvas readback blocked by file:// origin. Run \`python datasets/renoir-flowers/serve.py\` and reopen at http://localhost:${SYNCED_PORT}/gallery.html`
